...
export default function JDClient({ initialUser }: { initialUser: AccountUser }) {
  const [records, setRecords] = useState<Record<string, unknown>[]>([]);
  const [state, setState] = useState<LoadState>("idle");
  const [errorMessage, setErrorMessage] = useState("");
  const [selectedJob, setSelectedJob] = useState<JobDescription | null>(null);
  const [search, setSearch] = useState("");
  const [sessionUser, setSessionUser] = useState<AccountUser | null>(initialUser);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-sky-100 via-indigo-100 to-cyan-100 text-slate-800">
      <TopNav user={sessionUser ? { name: sessionUser.fullName || sessionUser.username } : null} onLogout={() => { setSessionUser(null); window.localStorage.removeItem("hrms_session_user"); void fetch("/api/auth/logout", { method: "POST" }).finally(() => { window.location.replace("/login"); }); }} />
